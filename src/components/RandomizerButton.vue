...
<template>
  <div class="relative">
    ...
    >
      <!-- Animated background -->
      <div class="absolute inset-0 bg-gradient-to-r from-primary-400 via-secondary-400 to-accent-400 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
    ...
    <!-- Tooltip -->
    <div class="absolute -bottom-2 left-1/2 transform -translate-x-1/2 translate-y-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-10">
      <div class="bg-neutral-900 text-white text-xs px-3 py-2 rounded-lg shadow-xl whitespace-nowrap">
        <div class="font-medium">Smart Banner Generator</div>
        <div class="text-neutral-300">Uses color theory for beautiful results</div>
        <div class="absolute -top-1 left-1/2 transform -translate-x-1/2 w-2 h-2 bg-neutral-900 rotate-45"></div>
      </div>
    </div>
  </div>
</template>
