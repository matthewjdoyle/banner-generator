  ...
  <div class="space-y-6">
    ...
    >
      ...
      <div class="space-y-4">
        ...
        <!-- Icon Grid -->
        <div>
          <label class="block text-sm font-semibold text-accent-800 mb-3"
            >Available Icons ({{ filteredIcons.length }})</label
          >
          <div
            class="grid grid-cols-6 gap-2 max-h-48 overflow-y-auto bg-white rounded-xl p-3 border border-accent-200"
          >
            <button
              v-for="icon in filteredIcons"
              :key="icon.id"
              @click="addIcon(icon.id)"
              class="group aspect-square p-1 hover:bg-accent-50 transition-all duration-300 flex items-center justify-center rounded-lg"
              :title="icon.name"
            >
              <!-- SVG Shape Preview -->
              <svg
              ...
              >
                ...
                <!-- Hexagon -->
                <polygon
                  v-else-if="icon.svg === 'hexagon'"
                  points="17.5,5.5 17.5,18.5 12,22 6.5,18.5 6.5,5.5 12,2"
                />
                ...
                <!-- Star -->
                <polygon
                  v-else-if="icon.svg === 'star'"
                  points="12,2 15,9 22,9 17,14 19,22 12,18 5,22 7,14 2,9 9,9"
                />

                <!-- Heart -->
                <path
                  v-else-if="icon.svg === 'heart'"
                  d="M12,21.35l-1.45-1.32C5.4,15.36,2,12.28,2,8.5 2,5.42,4.42,3,7.5,3c1.74,0,3.41,0.81,4.5,2.09C13.09,3.81,14.76,3,16.5,3 19.58,3,22,5.42,22,8.5c0,3.78-3.4,6.86-8.55,11.54L12,21.35z"
                />

                <!-- Arrow Right -->
                <path
                  v-else-if="icon.svg === 'arrow-right'"
                  d="M8,5v3h8l-3-3 1.5-1.5L20,9l-5.5,5.5L13,13l3-3H8v3l-3-3L8,5z"
                />

                <!-- Arrow Left -->
                <path
                  v-else-if="icon.svg === 'arrow-left'"
                  d="M16,19v-3H8l3,3-1.5,1.5L4,15l5.5-5.5L11,11l-3,3h8v-3l3,3L16,19z"
                />

                <!-- Arrow Up -->
                <path
                  v-else-if="icon.svg === 'arrow-up'"
                  d="M19,16h-3v-8l3,3 1.5-1.5L15,4 9.5,9.5 11,11l3-3v8H11l3,3L19,16z"
                />

                <!-- Arrow Down -->
                <path
                  v-else-if="icon.svg === 'arrow-down'"
                  d="M5,8h3v8l-3-3-1.5,1.5L9,20l5.5-5.5L13,13l-3,3V8h3l-3-3L5,8z"
                />
                ...
                <!-- Cross -->
                <path
                  v-else-if="icon.svg === 'cross'"
                  d="M19,6.41L17.59,5 12,10.59 6.41,5 5,6.41 10.59,12 5,17.59 6.41,19 12,13.41 17.59,19 19,17.59 13.41,12 19,6.41z"
                />

                <!-- Check -->
                <path
                  v-else-if="icon.svg === 'check'"
          ...
          <p class="text-xs text-accent-700 mt-2 text-center">
            Click any icon to add it to your banner
          </p>
        </div>
      </div>
    </div>
          ...
          <div class="flex items-center justify-between relative z-10">
            <div class="flex items-center space-x-3">
              <div class="w-10 h-10 bg-neutral-100 rounded-xl flex items-center justify-center">
                ...
                <svg
                  v-if="
                    bannerStore.availableIcons
                      .find((i) => i.id === icon.type)
                      ?.svg?.startsWith('svg-') ||
                    [
                      'circle',
                      'square',
                      ...
                      bannerStore.availableIcons.find((i) => i.id === icon.type)?.svg || '',
                    )
                  "
                  width="20"
                  height="20"
                ...
                >
                  ...
                  <circle
                    v-if="
                      bannerStore.availableIcons.find((i) => i.id === icon.type)?.svg === 'circle'
                    "
                    cx="12"
                    cy="12"
                    r="8"
                  ...
                  <rect
                    v-else-if="
                      bannerStore.availableIcons.find((i) => i.id === icon.type)?.svg === 'square'
                    "
                    x="4"
                    y="4"
                    width="16"
                  ...
                  <polygon
                    v-else-if="
                      bannerStore.availableIcons.find((i) => i.id === icon.type)?.svg === 'triangle'
                    "
                  ...
                  <polygon
                    v-else-if="
                      bannerStore.availableIcons.find((i) => i.id === icon.type)?.svg === 'diamond'
                    "
                  ...
                  <polygon
                    v-else-if="
                      bannerStore.availableIcons.find((i) => i.id === icon.type)?.svg === 'hexagon'
                    "
                    points="17.5,5.5 17.5,18.5 12,22 6.5,18.5 6.5,5.5 12,2"
                  />
                  ...
                  <polygon
                    v-else-if="
                      bannerStore.availableIcons.find((i) => i.id === icon.type)?.svg === 'pentagon'
                    "
                    points="12,2 22,9 18,20 6,20 2,9"
                  />

                  <!-- Star -->
                  <polygon
                    v-else-if="
                      bannerStore.availableIcons.find((i) => i.id === icon.type)?.svg === 'star'
                    "
                    points="12,2 15,9 22,9 17,14 19,22 12,18 5,22 7,14 2,9 9,9"
                  />
                  ...
                  <path
                    v-else-if="
                      bannerStore.availableIcons.find((i) => i.id === icon.type)?.svg === 'heart'
                    "
                    d="M12,21.35l-1.45-1.32C5.4,15.36,2,12.28,2,8.5 2,5.42,4.42,3,7.5,3c1.74,0,3.41,0.81,4.5,2.09C13.09,3.81,14.76,3,16.5,3 19.58,3,22,5.42,22,8.5c0,3.78-3.4,6.86-8.55,11.54L12,21.35z"
                  />
                  ...
                  <path
                    v-else-if="
                      bannerStore.availableIcons.find((i) => i.id === icon.type)?.svg === 'plus'
                    "
                  ...
                  <path
                    v-else-if="
                      bannerStore.availableIcons.find((i) => i.id === icon.type)?.svg === 'cross'
                    "
                    d="M19,6.41L17.59,5 12,10.59 6.41,5 5,6.41 10.59,12 5,17.59 6.41,19 12,13.41 17.59,19 19,17.59 13.41,12 19,6.41z"
                  />
                  ...
                  <path
                    v-else-if="
                      bannerStore.availableIcons.find((i) => i.id === icon.type)?.svg === 'check'
                    "
                    d="M9,16.17L4.83,12l-1.42,1.41L9,19 21,7l-1.41-1.41L9,16.17z"
                  />
                </svg>
              ...
              <div>
                <p
                  class="font-semibold text-neutral-900 group-hover:text-primary-600 transition-colors"
                >
                ...
                <div class="flex items-center space-x-2 text-sm text-neutral-600 mt-1">
                  ...
                  <span>•</span>
                  <span v-if="icon.colorType === 'gradient'" class="text-accent-600 font-medium"
                    >Gradient</span
                  >
    ...
    >
      <div
        class="bg-gradient-to-r from-primary-100 to-primary-200/80 -m-6 mb-6 p-6 rounded-t-2xl border-b border-primary-300/50"
      >
        <div class="flex items-center space-x-3">
          <div
      ...
      <div class="space-y-6">
          ...
          <div class="space-y-4">
                  ...
                  @input="
                    updateIcon({
                      gradientColors: [
                        ...
                        selectedIcon.gradientColors?.[1] || '#1E40AF',
                      ],
                    })
                  "
              ...
              <div>
                <input
                  ...
                  @input="
                    updateIcon({
                      gradientColors: [
                        selectedIcon.gradientColors?.[0] || '#3B82F6',
                        ($event.target as HTMLInputElement).value,
                      ],
                    })
                  "
            ...
            <div>
              <input
                ...
                class="w-full"
              />
              <div class="text-center text-xs text-primary-600 mt-1">
                {{ selectedIcon.gradientDirection || 45 }}°
              </div>
            </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <input
                ...
                class="w-full"
              />
              <div class="text-center text-xs text-primary-600 mt-1">X: {{ selectedIcon.x }}px</div>
            </div>
            <div>
              <input
                type="range"
                ...
                class="w-full"
              />
              <div class="text-center text-xs text-primary-600 mt-1">Y: {{ selectedIcon.y }}px</div>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            <input
              ...
              @input="
                updateIcon({ opacity: parseFloat(($event.target as HTMLInputElement).value) })
              "
              min="0"
              max="1"
              step="0.1"
            ...
            />
            <div class="text-center text-xs text-primary-600 mt-1">
              Opacity: {{ Math.round((selectedIcon.opacity || 1) * 100) }}%
            </div>
          </div>
        </div>
        ...
        <div>
          <div class="space-y-4">
            <div class="flex items-center justify-between">
              <span class="text-sm font-medium text-primary-700">Shadow</span>
              <button
                @click="
                  updateIcon({
                    shadow: { ...selectedIcon.shadow, enabled: !selectedIcon.shadow.enabled },
                  })
                "
                :class="[
            ...
            <div v-if="selectedIcon.shadow.enabled" class="space-y-3">
              ...
              <div class="grid grid-cols-3 gap-3">
                <div>
                  ...
                  />
                  <div class="text-center text-xs text-primary-600 mt-1">
                    Blur: {{ selectedIcon.shadow.blur }}px
                  </div>
                </div>
                <div>
                  <input
                  ...
                  />
                  <div class="text-center text-xs text-primary-600 mt-1">
                    X: {{ selectedIcon.shadow.offsetX }}px
                  </div>
                </div>
                <div>
                  <input
                  ...
                  />
                  <div class="text-center text-xs text-primary-600 mt-1">
                    Y: {{ selectedIcon.shadow.offsetY }}px
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="flex space-x-3 pt-4 border-t border-primary-200">
          <button @click="duplicateIcon" class="btn-secondary flex-1">📋 Duplicate</button>
          <button @click="removeSelectedIcon" class="btn-danger flex-1">🗑️ Delete</button>
        </div>
      </div>
    </div>
